...
import { ExternalLink, Globe, Loader2, Lock, Save } from "lucide-react";
...
export function TrainingUrlPage() {
  const { data: datasets, isLoading } = useDatasets();
  const { data: savedTrainingUrl } = useTrainingUrl();
  ...
  const effectiveUrl = trainingUrl.trim() || savedTrainingUrl?.trim() || "";
  ...
  const handleOpenExternal = (dataset: Dataset) => {
    if (!effectiveUrl) {
      toast.error("Set a training platform URL first");
      return;
    }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      >
        ...
        <p className="text-muted-foreground text-sm">
          Set your AI training platform URL and launch verified datasets
        </p>
      </motion.div>
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">
          ...
          HuggingFace, Colab, SageMaker). The dataset ID, hash, and a secure
          download link will be appended automatically when you launch.
        </p>
        ...
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-muted-foreground">
            Examples: HuggingFace, Google Colab, AWS SageMaker, Kaggle
          </p>
        </div>
      ...
      >
        ...
        {verifiedDatasets.length === 0 ? (
          <div
            data-ocid="training.datasets.empty_state"
          ...
          <div className="space-y-3">
            {verifiedDatasets.map((ds, idx) => (
              <motion.div
                ...
                data-ocid={`training.datasets.item.${idx + 1}`}
              >
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleOpenExternal(ds)}
                  className="border-border text-xs h-8 px-3 hover:border-primary/50 hover:text-primary shrink-0 ml-3"
                  data-ocid={`training.external_link_button.${idx + 1}`}
                  title="Verify dataset and open external platform"
                >
